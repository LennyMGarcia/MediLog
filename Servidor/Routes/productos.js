...
const validaciones = [
    ...
    body('categoria').escape().trim().isString().notEmpty().withMessage('Categoria Obligatoria.'),
    body('precio').escape().trim().isFloat({ min: 0 }).notEmpty().withMessage('Precio Obligatorio.'),
];
const id_validation = [
...
router.put('/:id', id_validation, async (req, res) => {
    ...
    if (validated.isEmpty()) {
        const data = req.body;
        if (!data.nombre || !data.categoria || !data.precio) return res.status(400).json({ 'message': 'Campos Obligatorios.' });
